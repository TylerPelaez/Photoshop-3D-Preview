...
/** The Task class can be used to implement tasks that are asynchronous in nature.
 An example of such a task is something that needs to invoke APIs on the main thread.
 The general flow of a Task is that the JavaScript thread creates the task and an
 associated deferred value. This value is returned to JavaScript thread and becomes and awaitable
 promise.
 When the task is complete, then it must schedule a promise resolution on the JavaScript thread.
*/
